...
import type { PersonalizationScore } from '../types';
import { PersonalizationIcon, ChevronDownIcon } from './icons/CategoryIcons';
import { InfoTooltip } from './InfoTooltip';

interface PersonalizationScoreCardProps {
  scoreData: PersonalizationScore;
}

export const PersonalizationScoreCard: React.FC<PersonalizationScoreCardProps> = ({ scoreData }) => {
      ...
      <div className="flex-grow flex flex-col md:flex-row items-center gap-4 mt-4">
        <div className="flex-shrink-0">
          <div className="relative w-20 h-20">
            ...
            <span className={`absolute inset-0 flex items-center justify-center text-xl font-bold ${colorClasses}`} data-testid="text-personalization-score">
              {scoreData.score}
            </span>
          </div>
        </div>
