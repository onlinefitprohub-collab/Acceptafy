...
const getSeverityStyles = (severity: 'High' | 'Medium' | 'Low'): { pill: string; dot: string } => {
  switch (severity) {
    case 'High':
      return { pill: 'bg-red-500/20 text-red-300', dot: 'bg-red-400' };
    case 'Medium':
      return { pill: 'bg-yellow-500/20 text-yellow-300', dot: 'bg-yellow-400' };
    case 'Low':
      return { pill: 'bg-sky-500/20 text-sky-300', dot: 'bg-sky-400' };
    default:
      return { pill: 'bg-gray-500/20 text-gray-300', dot: 'bg-gray-400' };
  }
};
...
export const AccessibilityCard: React.FC<AccessibilityCardProps> = ({ findings }) => {
  ...
  return (
    <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-2xl shadow-lg" data-testid="accessibility-card">
      <button
        type="button"
      ...
      >
        <div className="flex items-center gap-3">
          <span className="text-purple-400"><AccessibilityIcon /></span>
          <h3 className="text-lg sm:text-xl font-semibold text-white">Accessibility & Inclusivity</h3>
           ...
           <div className="px-4 sm:px-6 pb-4 sm:pb-6">
            <div className="border-t border-white/10 pt-4 mt-4 space-y-4">
              {findings.map((finding, index) => {
                ...
                return (
                  <div key={index} className="bg-white/5 p-4 rounded-lg border border-white/10" data-testid={`accessibility-finding-${index}`}>
                    <div className="flex justify-between items-start mb-3">
                        <h4 className="text-lg font-bold text-purple-300">{finding.type}</h4>
                    ...
                    <div className="space-y-2 text-sm">
                        <p className="text-gray-400 italic">"{finding.summary}"</p>
                        <div>
                            <h5 className="font-semibold text-gray-200">Suggestion:</h5>
                            <p className="text-gray-300">{finding.suggestion}</p>
                        </div>
                    </div>
                  </div>
                );
