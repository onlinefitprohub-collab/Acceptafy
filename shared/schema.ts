...
// Content Calendar / Scheduled Campaigns table
export const scheduledCampaigns = pgTable("scheduled_campaigns", {
  ...
  category: varchar("category"), // welcome, newsletter, promotional, etc.
  tags: text("tags").array(),
  // ESP/sending info
  espProvider: varchar("esp_provider"),
  espCampaignId: varchar("esp_campaign_id"),
  ...
  // Visual display
  color: varchar("color").default("#9333ea"),
  // Timestamps
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
  sentAt: timestamp("sent_at"),
...
// Calendar view event helper type
