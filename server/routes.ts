...
export async function registerRoutes(
  ...
  // Content Calendar / Scheduled Campaigns Routes
  app.get('/api/calendar/campaigns', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      ...
      const campaigns = await storage.getScheduledCampaigns(userId, startDate, endDate);
      
      const calendarEvents = campaigns.map(c => ({
        id: c.id,
        title: c.title,
        ...
        openRate: c.openedCount && c.sentCount ? (c.openedCount / c.sentCount) * 100 : undefined,
        clickRate: c.clickedCount && c.sentCount ? (c.clickedCount / c.sentCount) * 100 : undefined,
      }));
      
      res.json(calendarEvents);
    } catch (error) {
      console.error('Calendar campaigns error:', error);
      res.status(500).json({ error: 'Failed to fetch calendar campaigns' });
    }
  });

  app.get('/api/calendar/campaigns/:id', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const { id } = req.params;
      
      const campaign = await storage.getScheduledCampaign(id, userId);
      if (!campaign) {
  ...
  app.post('/api/calendar/campaigns', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const { title, description, subject, previewText, body, scheduledDate, scheduledTime, timezone, category, tags, color, listId, listName } = req.body;
  ...
  app.put('/api/calendar/campaigns/:id', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const { id } = req.params;
      ...
      const campaign = await storage.updateScheduledCampaign(id, userId, updates);
      if (!campaign) {
        return res.status(404).json({ error: 'Campaign not found' });
      }
  ...
  app.delete('/api/calendar/campaigns/:id', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const { id } = req.params;
      
      const deleted = await storage.deleteScheduledCampaign(id, userId);
